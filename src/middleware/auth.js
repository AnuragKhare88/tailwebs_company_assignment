...
const authorization = async function (req, res, next) {
  try {
    ...
    const studentByStudentId = await StudentModel.findOne({_id: studentId,isDeleted: false,deletedAt: null,});

    if (!studentByStudentId) {
      return res.status(404).send({ status: false, message: `no student found by ${studentId}`});
    }
