...
const createStudent = async function (req, res) {
    ...
    try {
        ...
        if (!["English", "Hindi", "Maths", "Science"].includes(subject)) {
            return res.status(400).send({ status: false, message: "subject is required and its value must be either English/Hindi/Maths/Science" });
        }
        ...
        if (studentDataByNameMarks) {
            ...
            let studentData = await studentModel.findOneAndUpdate({ _id: studentId },{$inc:{marks: marks},}, { new: true, upsert: true });

            return res.status(201).send({ status: true, message: `student already exits with ${subject} so marks updated`, data: studentData })

        } else {

            const newStudent = await studentModel.create(requestBody)

            return res.status(201).send({ status: true, message: "new student entry done", data: newStudent })
}
} catch (error) {
...
const studentList = async function (req, res) {
    ...
    try {
        ...
        const filterConditions = {teacherId: teachetByTeacherId,isDeleted: false};
        ...
        if (isValidRequestBody(queryParams)) {
            ...
            if (queryParams.hasOwnProperty("subject")) {
                if (isValid(subject) && ["English", "Hindi", "Maths", "Science"].includes(subject)) {
                    ...
                    return res.status(400).send({ status: false, message: `enter a valid subject and it should be either English/Hindi/Maths/Science` });
                }
            }
            ...
            return res.status(200).send({ status: true, message: `Filtered students of Teacher:- ${teacherName}`, studentCount: studentListAfterFiltration.length, studentList: studentListAfterFiltration });
        } else {
            ...
            return res.status(200).send({ status: true, message: `All students of Teacher:- ${teacherName}`, studentCount: studentList.length, studentList: studentList, });
        }
    } catch (err) {
...
const updateStudent = async function (req, res) {
    try {
        ...
        if (!studentId) {
            return res.status(400).send({ status: false, message: "studentId is required in path params" });
        }
        ...
        if (!studentBystudentId) {
            return res.status(400).send({ status: false, message: `no student exits with id ${studentId}` })
        }
        ...
        if (requestBody.hasOwnProperty("name")) {
            if (isValid(name)) {
                ...
                return res.status(400).send({ status: false, message: `enter name in valid format` });
            }
        }
        ...
        if (requestBody.hasOwnProperty("subject")) {
            ...
            if (isValid(subject) && ["English", "Hindi", "Maths", "Science"].includes(subject)) {
                ...
                return res.status(400).send({ status: false, message: "enter subject in valid format and it should be either English/Hindi/Maths/Science" });
            }
        }
        ...
        if (requestBody.hasOwnProperty("marks")) {
            if (/^0*[1-9]\d*$/.test(marks)) {
                ...
                return res.status(400).send({ status: false, message: "give valid marks" });
            }
        }
        ...
        return res.status(200).send({ status: true, message: "student details update successfully", data: updatedStudentDetails });
...
const deleteStudent = async function (req, res) {
    ...
    try {
        ...
        if (!studentData) {
            return res.status(404).send({ status: false, message: `no student found by this ${studentId}` })
        }

        await studentModel.findByIdAndUpdate(studentId,{$set:{ isDeleted:true}},{new:true});
        return res.status(200).send({ status: true, message: "student Detail deleted successfully" });
...
module.exports = {createStudent, studentList, updateStudent, deleteStudent}
