...
const createTeacher = async function (req, res) {
  try {
    ...
    if (Object.keys(requestBody).length > 3) {
      return res.status(400).send({status: false, message: "invalid data entered inside request body",});
    }
    ...
    if (isNameNotUnique) {
      return res.status(409).send({ status: false, message: "name already exits" });
    }
    ...
    if (isemailNotUnique) {
      return res.status(409).send({ status: false, message: "email already exits" });
    }
    ...
    return res.status(201).send({status: true,message: "new Teacher entry done",data: newTeacherEntry});
  } 
  catch (error) {
...
const teacherLogin = async function (req, res) {
  try {
    ...
    if (!teacherDetails) {
      return res.status(404).send({ status: false, message: "no teacher found by email" });
    }

    const isPasswordMatching = await bcrypt.compare(password,teacherDetails.password);
